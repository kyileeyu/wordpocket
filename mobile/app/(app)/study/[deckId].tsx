...
import { useStudyQueue, useReviewBatch } from "@/hooks/useStudy";
import { computeIntervals, timeAgo } from "@/lib/utils";
...
export default function StudyPage() {
  ...
  useEffect(() => {
    if (queue && queue.length > 0 && !initRef.current) {
      setWorkingQueue(queue as StudyCard[]);
  ...
  const intervals = useMemo(() => {
    ...
    return computeIntervals({ status: card.status, ease_factor: card.ease_factor, interval: card.interval, step_index: card.step_index });
  ...
  const handleResponse = async (label: string) => {
    ...
    if (nextIndex >= nextQueue.length) {
      ...
      if (serverQueue && serverQueue.length > 0) {
        setWorkingQueue(serverQueue as StudyCard[]);
        setIndex(0); setFlipped(false);
        setLapStartIndex(0); setLapEndIndex(serverQueue.length);
      } else {
        router.replace({
          pathname: "/(app)/study/complete",
          params: { reviewed: String(nextReviewed), correct: String(nextCorrect), newCount: String(newCount) },
        });
      }
    } else {
  ...
  if (isLoading) {
    return (
      <SafeAreaView className="flex-1 bg-bg-primary px-6 gap-4 pt-16">
  ...
  if (!queue || queue.length === 0) {
    return (
      <SafeAreaView className="flex-1 bg-bg-primary items-center justify-center">
        <EmptyState icon="📚" text="오늘 학습할 카드가 없습니다." />
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-bg-primary">
      ...
      <View className="flex-row items-center justify-between px-4 py-2">
        ...
        <Text className="text-mono-md text-text-secondary">
      ...
      <View className="flex-1 justify-center px-6">
        {card.last_reviewed_at && (
          <Text className="text-caption text-text-tertiary text-right mb-1 pr-3">
